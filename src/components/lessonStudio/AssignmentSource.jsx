...
// Fast assignment intake. Selecting the learners is optional; the normal path is
// simply drop the work and create. Analysis is returned to LessonStudio, which can
// immediately generate the full plan without another Continue screen.
export default function AssignmentSource({ students, onComplete, analyzing, onAnalyzing, autoGenerate = false }) {
  ...
  const uploadFile = async (file) => {
    ...
    try {
      ...
      toast({ title: "Assignment ready", description: file.name });
      return res.file_url;
    } catch (err) {
  ...
  const analyze = async () => {
    if (!fileUrl && !pastedText.trim() && !pageUrl.trim()) {
      toast({ title: "Add today's work", description: "Drop a worksheet, choose a file, paste the work, or add a link.", variant: "destructive" }); return;
    }
    onAnalyzing(true);
    try {
      ...
      toast({ title: "Could not read assignment", description: e?.response?.data?.error || e.message, variant: "destructive" });
  ...
  return <div className="space-y-5">
    <Card className="p-5 sm:p-6">
      <div className="flex items-center justify-between gap-3 mb-4">
        <div><h3 className="font-semibold flex items-center gap-2"><Users className="h-4 w-4 text-primary"/>Who are you teaching?</h3><p className="text-xs text-muted-foreground mt-1">Optional. Select students so CaseCue can match their verified IEP goals and accommodations.</p></div>
      </div>
    ...
    <Card className="p-5 sm:p-6">
      ...
      <div onDragEnter={(e)=>{e.preventDefault();setDragging(true)}} onDragOver={(e)=>{e.preventDefault();setDragging(true)}} onDragLeave={(e)=>{e.preventDefault();setDragging(false)}} onDrop={onDrop} onClick={()=>!uploading&&inputRef.current?.click()} className={cn("rounded-2xl border-2 border-dashed p-8 sm:p-10 text-center cursor-pointer transition",dragging?"border-primary bg-primary/5":"border-border hover:border-primary/50 hover:bg-muted/30")}>
        <input ref={inputRef} type="file" className="hidden" accept={ACCEPT} onChange={onUpload}/>
        {uploading ? <Loader2 className="h-9 w-9 mx-auto animate-spin text-primary"/> : fileName ? <CheckCircle2 className="h-9 w-9 mx-auto text-emerald-600"/> : <Upload className="h-9 w-9 mx-auto text-primary"/>}
        <p className="font-semibold mt-3">{uploading?"Uploading…":fileName?fileName:"Drag & drop today's assignment here"}</p><p className="text-xs text-muted-foreground mt-1">or click to choose PDF, Word, photo, scan or text file</p>
      </div>
      <details className="mt-4"><summary className="text-sm font-medium cursor-pointer text-muted-foreground">Other ways to add work</summary><div className="space-y-4 mt-4"><textarea className="w-full rounded-lg border border-input p-3 min-h-[90px]" value={pastedText} onChange={e=>setPastedText(e.target.value)} placeholder="Paste assignment text or type the skill you are teaching…"/><div className="flex items-center gap-2"><Link2 className="h-4 w-4 text-muted-foreground"/><input className={inputCls} value={pageUrl} onChange={e=>setPageUrl(e.target.value)} placeholder="Webpage or YouTube link"/></div></div></details>
      ...
      <p className="text-xs text-muted-foreground mt-2">CaseCue analyzes the work, matches verified student information, and builds the detailed plan. Review before instructional use.</p>
    </Card>
  </div>;
}