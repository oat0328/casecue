...
export default function BaselineAssessmentStudio({student}){
  ...
  const saveScores=async()=>{
    if(!current)return;const items=allItems.map(i=>({item_id:i.id,earned_points:scored[i.id]?.earned_points===''||scored[i.id]?.earned_points==null?null:Math.max(0,Math.min(Number(i.max_points||0),Number(scored[i.id]?.earned_points))),teacher_note:scored[i.id]?.teacher_note||''})).filter(x=>x.earned_points!=null);
    if(!items.length)return toast({title:'Enter at least one scored item',variant:'destructive'});
    setBusy(true);try{await base44.entities.BaselineAssessment.update(current.id,{results:{items},status:'completed'});await load(current.id);toast({title:'Baseline scores saved',description:`${items.length} scored item${items.length===1?'':'s'} saved. CaseCue will only analyze what you scored.`});}catch(e){toast({title:'Could not save scores',description:e.message,variant:'destructive'})}finally{setBusy(false)}
  };
  const analyze=async()=>{
    if(!current)return;setBusy(true);try{await saveScores();const r=await base44.functions.invoke('evaluateBaselineAssessment',{assessment_id:current.id});const analysis=r.data?.analysis||r.analysis;if(!analysis)throw new Error('No analysis was returned.');await base44.entities.BaselineAssessment.update(current.id,{analysis,status:'analyzed'});await load(current.id);toast({title:'Baseline evaluation ready',description:'Present levels and measurable goal options are ready for educator review.'});}catch(e){toast({title:'Could not evaluate baseline',description:e?.response?.data?.error||e.message,variant:'destructive'})}finally{setBusy(false)}
  };
