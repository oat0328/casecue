...
export default function SessionImportPanel({ students = [], goals = [], sessions = [], onImported }) {
  ...
  const serviceCategoriesFor = (student) => (student?.services || []).map(goalCategory).filter(Boolean);
  const auditRows = useMemo(() => rows.map(r => {
    const student = students.find(s => s.id === r.student_id);
    const sessionCat = goalCategory(`${r.goal_text || ''} ${r.activity || ''}`);
    const serviceCats = serviceCategoriesFor(student);
    const hasServiceDetails = (student?.services || []).length > 0 || Number(student?.service_minutes || 0) > 0;
    const serviceMismatch = !!(r.student_id && sessionCat && hasServiceDetails && serviceCats.length && !serviceCats.includes(sessionCat));
    const testingLike = /\b(map|testing|assessment|diagnostic|benchmark|ixl)\b/i.test(`${r.activity || ''} ${r.goal_text || ''}`);
    const goalEvidence = !!r.goal_id && !testingLike && (r.correct != null || r.total != null || r.percentage != null || !!r.qualitative);
    const reviewReasons = [];
    if (!r.student_id) reviewReasons.push('Student is not matched to the active caseload');
    if (!r.date) reviewReasons.push('Session date is missing');
    if (r.minutes_review) reviewReasons.push('Delivered minutes are unusually high');
    if (serviceMismatch) reviewReasons.push('Session area is not listed in this student’s current CaseCue services');
    return { ...r, service_mismatch: serviceMismatch, testing_like: testingLike, goal_evidence: goalEvidence, audit_reasons: reviewReasons };
  }), [rows, students]);
  const ready = auditRows.filter(r => r.student_id && r.date && !r.duplicate && !r.minutes_review && !r.service_mismatch);
  const alreadyImported = auditRows.filter(r => r.duplicate).length;
  const reviewRows = auditRows.filter(r => !r.duplicate && (r.audit_reasons || []).length);
  ...
  const serviceMismatchCount = reviewRows.filter(r => r.service_mismatch).length;
  const safeGoalEvidenceCount = auditRows.filter(r => !r.duplicate && r.goal_evidence).length;
  const testingRowsCount = auditRows.filter(r => !r.duplicate && r.testing_like).length;
  ...
  const doImport = async () => {
    ...
    try {
      ...
      const progressRecords = freshReady.filter(r => r.goal_evidence).map(r => ({ organization_id: org, student_id: r.student_id: goal_id: r.goal_id, date: r.date, correct: r.correct ?? undefined, total: r.total ?? undefined, percentage: r.percentage ?? undefined, decimal: r.percentage != null ? Math.round((r.percentage / 100) * 100) / 100 : undefined, qualitative_notes: r.qualitative || undefined, observation_notes: r.qualitative || undefined, prompting_level: 'independent' }));
  ...
  return <div className="space-y-4">
    ...
    {rows.length > 0 && <Card className="overflow-hidden"><div className="px-5 py-4 border-b flex flex-col md:flex-row md:items-center md:justify-between gap-3"><div><div className="font-black">Review import — {fileName}</div><div className="text-sm text-slate-500">{ready.length} new sessions ready · {alreadyImported} already imported · {needsReview} need review</div><div className="text-xs text-emerald-700 mt-1">Integrity audit: {safeGoalEvidenceCount} verified goal-evidence rows · {testingRowsCount} testing/diagnostic rows kept out of automatic progress evidence</div>{needsReview > 0 && <div className="text-xs text-amber-700 mt-1">Review breakdown: {missingStudentCount} student match · {missingDateCount} missing date · {unusualMinutesCount} over 120 minutes · {serviceMismatchCount} service-area mismatch</div>}</div><Button onClick={doImport} disabled={busy || !ready.length} className="bg-blue-700 hover:bg-blue-800 text-white">Approve & Import {ready.length} Sessions</Button></div><div className="overflow-x-auto max-h-[560px]"><table className="w-full text-sm min-w-[1100px]"><thead className="sticky top-0 bg-slate-50 border-b"><tr className="text-left text-xs text-slate-500"><th className="px-4 py-3">Row</th><th className="px-4 py-3">Student</th><th className="px-4 py-3">Date</th><th className="px-4 py-3">Minutes</th><th className="px-4 py-3">Activity</th><th className="px-4 py-3">Goal</th><th className="px-4 py-3">Score</th><th className="px-4 py-3">Status</th></tr></thead><tbody className="divide-y">{auditRows.map((r, i) => <tr key={`${r.row}-${i}`} className={r.duplicate ? 'bg-emerald-50/50' : (r.audit_reasons || []).length ? 'bg-rose-50/50' : ''}><td className="px-4 py-3 align-top">{r.row}{r.duplicate && <div className="text-[10px] text-emerald-700 font-bold mt-1">Already imported</div>}{!r.duplicate && !r.student_id && <div className="text-[10px] text-rose-700 font-bold mt-1">Student match</div>}{!r.duplicate && !r.date && <div className="text-[10px] text-rose-700 font-bold mt-1">Missing date</div>}{!r.duplicate && r.minutes_review && <div className="text-[10px] text-amber-700 font-bold mt-1">Review minutes</div>}{!r.duplicate && r.service_mismatch && <div className="text-[10px] text-rose-700 font-bold mt-1">Service mismatch</div>}</td><td className="px-4 py-3 align-top w-56"><Select value={r.student_id || undefined} onValueChange={sid => patchRow(i, { student_id: sid, goal_id: goalFor(sid, r.goal_text) })}><SelectTrigger className={!r.student_id ? 'border-rose-300 bg-white' : ''}><SelectValue placeholder={r.student_name || 'Match student'} /></SelectTrigger><SelectContent>{students.map(s => <SelectItem key={s.id} value={s.id}>{s.first_name} {s.last_name}</SelectItem>)}</SelectContent></Select>{!r.student_id && <div className="text-[10px] text-rose-600 mt-1">Student match required</div>}</td><td className="px-4 py-3 align-top">{r.date || <span className="text-rose-600">Missing</span>}<div className="text-xs text-slate-400">{r.start_time}{r.end_time ? `–${r.end_time}` : ''}</div></td><td className="px-4 py-3 align-top">{r.delivered_minutes ?? r.duration_minutes ?? '—'}</td><td className="px-4 py-3 align-top max-w-[220px]"><div className="font-medium">{r.activity}</div><div className="text-xs text-slate-400">{r.qualitative}</div></td><td className="px-4 py-3 align-top w-52"><Select value={r.goal_id || 'none'} onValueChange={gid => patchRow(i, { goal_id: gid === 'none' ? '' : gid })}><SelectTrigger><SelectValue placeholder="Not goal-linked" /></SelectTrigger><SelectContent><SelectItem value="none">Not goal-linked</SelectItem>{goals.filter(g => g.student_id === r.student_id).map(g => <SelectItem key={g.id} value={g.id}>{g.goal_area || 'Goal'}</SelectItem>)}</SelectContent></Select></td><td className="px-4 py-3 align-top">{r.correct != null || r.total != null ? `${r.correct ?? '—'}/${r.total ?? '—'}` : r.percentage != null ? `${Math.round(r.percentage * 10) / 10}%` : '—'}</td><td className="px-4 py-3 align-top">{r.status}</td></tr>)}</tbody></table></div><div className="px-5 py-3 border-t bg-slate-50 text-xs text-slate-500 flex items-center gap-2"><AlertTriangle className="h-4 w-4 text-amber-600" />Rows marked Already imported are safely skipped. Unmatched students, missing dates, unusual minutes, and service-area mismatches stay in review. Testing/diagnostic rows remain session history but never create automatic IEP progress evidence. Only verified goal-linked evidence creates ProgressData.</div></Card>}
  </div>;
}
