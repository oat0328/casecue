...
export default function SessionImportPanel({ students = [], goals = [], sessions = [], onImported }) {
  ...
  const ready = rows.filter(r => r.student_id && r.date && !r.duplicate && !r.minutes_review);
  const alreadyImported = rows.filter(r => r.duplicate).length;
  const reviewRows = rows.filter(r => !r.duplicate && (!r.student_id || !r.date || r.minutes_review));
