    ...
    <div className="flex h-full flex-col bg-[#08111f] text-white">
      <div className="h-[72px] px-5 flex items-center border-b border-white/10">
        <Link to="/app" className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-indigo-500 to-sky-400 flex items-center justify-center shadow-lg shadow-indigo-950/30"><Sparkles className="h-5 w-5"/></div>
    ...
    <div className="min-h-screen bg-[#f4f7fb]">
      ...
      <div className="lg:pl-[272px] min-h-screen">
        <header className="sticky top-0 z-30 h-[72px] border-b border-slate-200/80 bg-white/90 backdrop-blur-xl px-4 sm:px-6 lg:px-8 flex items-center gap-3">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild><button className="flex items-center gap-2 rounded-xl border border-transparent px-1.5 py-1 hover:bg-slate-100"><Avatar className="h-9 w-9"><AvatarFallback className="bg-gradient-to-br from-indigo-500 to-sky-400 text-white text-xs font-bold">{initials}</AvatarFallback></Avatar><div className="hidden sm:block text-left"><div className="max-w-[150px] truncate text-sm font-semibold text-slate-900">{user?.full_name || user?.email}</div><div className="text-[11px] text-slate-500">CaseCue workspace</div></div><ChevronDown className="h-4 w-4 text-slate-400"/></button></DropdownMenuTrigger>
