...
import{UploadCloud,Loader2,History,FileStack,Trash2,CheckCircle2}from'lucide-react';
...
export default function BatchWorkEvidencePanel({students=[],goals=[],onSaved,v2=false,batchMode='guided'}){
 ...
 const openEvidence=async x=>{try{if(x.work_evidence_id){const r=await base44.functions.invoke('openEvidenceUrl',{evidence_id:x.work_evidence_id});const u=r?.data?.signed_url||r?.signed_url;if(u){window.open(u,'_blank','noopener,noreferrer');return;}}if(x.evidence_file_uri){const r=await base44.functions.invoke('openPrivateFileUrl',{file_uri:x.evidence_file_uri});const s=r?.data||r;if(s?.signed_url){window.open(s.signed_url,'_blank','noopener,noreferrer');return;}}if(file){const u=URL.createObjectURL(file);window.open(u,'_blank','noopener,noreferrer');setTimeout(()=>URL.revokeObjectURL(u),60000);return;}throw new Error('No viewable work file is attached.');}catch(e){toast({title:'Could not open work',description:e?.response?.data?.error||e.message,variant:'destructive'});}};
  ...
  try{
   ...
   for(const chunk of chunks){
    ...
    try{
     ...
     const raw=(r.data?.items||r.items||[]).map((x,i)=>{const abs=(x.source_pages||[]).map(n=>chunk.start-1+Number(n)).filter(Number.isFinite);return{...x,detected_title:cleanText(batchMode==='guided'?assignmentTitle:x.detected_title),subject:cleanText(batchMode==='guided'?assignmentSubject:x.subject),skills:(x.skills||[]).map(cleanText).filter(Boolean),source_pages:abs,pages:abs.length?('Source page'+(abs.length===1?' ':'s ')+abs.join(', ')):('Source pages '+chunk.start+'-'+chunk.end),_key:String(chunk.start)+'-'+String(i)+'-'+String(Date.now()),approved:false};});
     ...
     for(const x of found){
      try{
       if(prepared.src&&x.source_pages?.length)x.evidence_file_uri=await extractArtifact(prepared.src,x,'casecue-artifact-'+job.id+'-'+x.source_pages.join('-')+'.pdf');
 ...
 const save=async(includeGoalEvidence=false)=>{
  ...
  try{
   ...
   for(const x of selected){
    ...
    const existing=x.duplicate_fingerprint&&x.student_id?await base44.entities.WorkEvidence.filter({student_id:x.student_id,duplicate_fingerprint:x.duplicate_fingerprint},'-created_date',1):[];
   ...
   refetchRuns();setFlowStep('complete');toast({title:String(saved)+' assignment'+(saved===1?'':'s')+' filed',description:includeGoalEvidence?'Selected IEP evidence was added after teacher approval.':'Grades filed without creating IEP progress evidence.'});onSaved?.();
  }catch(e){toast({title:'Could not file batch',description:e.message,variant:'destructive'});}finally{setSaving(false);}
 };

 const openRun=r=>{setRunId(r.id);setFileUri(r.file_uri||'');setFile(null);setItems((r.results||[]).map((x,i)=>({...x,_key:x._key||String(r.id)+'-'+String(i)})));setProgress(r.page_count?{stage:r.status==='completed'?'Completed':'Saved batch',pages:r.processed_pages||0,totalPages:r.page_count||0}:null);setFlowStep(r.status==='completed'?'complete':'decisions');};
 ...
 return<div className='space-y-5'>
  {flowStep==='setup'&&<Card className='p-6'>
   ...
   <div className='mt-5 grid gap-4 md:grid-cols-2'><div><Label>Answer key {batchMode==='guided'?'(recommended)':'(optional)'}</Label><Textarea rows={3} value={answerKey} onChange={e=>setAnswerKey(e.target.value)} placeholder='Paste answers, or leave blank for deterministic math'/></div><div><Label>Rubric {batchMode==='guided'?'(for writing/subjective work)':'(optional)'}</Label><Textarea rows={3} value={rubric} onChange={e=>setRubric(e.target.value)} placeholder='Paste rubric if needed'/></div></div>
