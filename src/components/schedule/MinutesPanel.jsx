...
export default function MinutesPanel({students,entries,sessionLogs}){
 ...
 const overlapChecks=useMemo(()=>findConflicts(entries),[entries]);
 const conflicts=useMemo(()=>overlapChecks.filter(c=>!c.same_slot),[overlapChecks]);
 const multiService=useMemo(()=>overlapChecks.filter(c=>c.same_slot),[overlapChecks]);
 ...
 const rows=roster.map(s=>{
  ...
  const missing=required!=null?Math.max(0,required-deliv):null;
  const schedulingGap=required!=null?Math.max(0,required-sched):null;
  const status=required==null?'Needs IEP Minutes':sched<required?'Under Scheduled':deliv>=required?'Delivered':'In Progress';
  return{s,required,sched,deliv,missing,schedulingGap,status,conflicts:conflictCount[s.id]||0};
 });
 const styles={Delivered:'bg-emerald-100 text-emerald-700','In Progress':'bg-blue-100 text-blue-700','Under Scheduled':'bg-amber-100 text-amber-700','Needs IEP Minutes':'bg-rose-100 text-rose-700'};

 return <div className='space-y-4'>
  <Card className='p-4 border-blue-100 bg-blue-50/40'>
   <div className='font-semibold text-sm'>How these numbers work</div>
   <div className='text-xs text-slate-600 mt-1'>Required / wk comes from the student's IEP service minutes saved in CaseCue. Scheduled / wk comes from active Instruction & Schedule groups. Delivered this wk comes from Session Tracker logs dated this week. Missing is Required minus Delivered. Exact same-time subject labels are treated as multi-service review items, not scheduling conflicts.</div>
  </Card>

  <Card className='p-0 overflow-hidden'>
   <div className='overflow-x-auto'><table className='w-full text-sm'>
    <thead><tr className='border-b bg-muted/50 text-left'>{['Student','Required / wk','Scheduled / wk','Delivered this wk','Missing','Conflicts','Status'].map(x=><th key={x} className='px-4 py-3 font-semibold'>{x}</th>)}</tr></thead>
    <tbody>{rows.map(({s,required,sched,deliv,missing,schedulingGap,status,conflicts:c})=><tr key={s.id} className='border-b last:border-0'>
     <td className='px-4 py-3 font-medium'>{s.first_name} {s.last_name}{s.grade?` (Grade ${s.grade})`:''}</td>
     <td className='px-4 py-3'>{required!=null?`${required} min`:<button className='font-semibold text-blue-600 hover:underline' onClick={()=>navigate(`/students/${s.id}`)}>Add IEP minutes</button>}</td>
     <td className={`px-4 py-3 ${schedulingGap>0?'text-amber-700 font-semibold':''}`}>{sched} min{schedulingGap>0&&<div className='text-[10px] font-normal'>{schedulingGap} min under plan</div>}</td>
     ...
     <td className='px-4 py-3'><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${styles[status]}`}>{status}</span></td>
    </tr>)}</tbody>
   </table></div>
  </Card>
