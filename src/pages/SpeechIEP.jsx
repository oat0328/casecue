 ...
 return <div className="space-y-6">
  ...
  {selected&&<div className="grid gap-5 xl:grid-cols-[.7fr_1.3fr]">
   <div className="space-y-4">
    ...
    <Card className="p-5"><h3 className="font-black">My Contribution History</h3><div className="mt-3 space-y-2">{myRows.map(x=><div key={x.id} className="rounded-xl border p-3 text-sm"><div className="flex items-start justify-between gap-2"><div><b>{name(x.student_id)}</b><div className="text-xs text-slate-500">{x.status||'draft'}{x.submitted_at?` · ${new Date(x.submitted_at).toLocaleDateString('en-US',{month:'2-digit',day:'2-digit',year:'numeric'})}`:''}</div></div><Button size="icon" variant="ghost" onClick={()=>remove(x)}><Trash2 className="h-4 w-4 text-rose-500"/></Button></div></div>)}{!myRows.length&&<div className="text-sm text-slate-500">No speech contributions saved for this student yet.</div>}</div></Card>
   </div>
