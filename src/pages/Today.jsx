...
export default function Today(){
  ...
  const actionItems=useMemo(()=>{
    const out=[];
    noteNeeded.slice(0,2).forEach(s=>{const st=studentMap.get(s.student_id);out.push({title:'Complete session notes',detail:`${st?`${st.first_name} ${st.last_name}`:'Student'}${s.date?` · ${formatDate(s.date)}`:''}`,path:'/session-tracker'});});
    ...
    (tasks||[]).filter(t=>t.status==='open').slice(0,5-out.length).forEach(t=>out.push({title:t.title,detail:t.due_date?`Due ${formatDate(t.due_date)}`:'Open task',path:'/app'}));
  ...
  return <div className="space-y-5">
    <section className="rounded-[28px] bg-gradient-to-br from-[#0a1220] via-[#111b31] to-[#18264b] px-6 py-7 sm:px-8 sm:py-8 text-white shadow-xl shadow-slate-950/10 overflow-hidden relative">
      ...
      <div className="relative flex flex-col xl:flex-row xl:items-end xl:justify-between gap-6">
        <div><div className="text-xs font-black uppercase tracking-[.2em] text-sky-300">CaseCue DayFlow</div><h1 className="mt-2 text-3xl sm:text-4xl font-black tracking-tight">Good morning, {name}.</h1><p className="mt-2 max-w-2xl text-slate-300">See what matters today, what is coming next, and what still needs your attention.</p></div>
