...
export default function Gradebook() {
  ...
  const openAssignment = async (a) => { try { let uri=a.file_url; if(!uri&&a.work_evidence_id){const ev=await base44.entities.WorkEvidence.filter({id:a.work_evidence_id},'-created_date',1);uri=ev?.[0]?.file_url;} if(!uri)throw new Error('No assignment image/PDF is linked to this grade yet.'); const r=await base44.functions.invoke('openPrivateFileUrl',{file_uri:uri,record_type:a.work_evidence_id?'WorkEvidence':'GradebookAssignment',record_id:a.work_evidence_id||a.id});const s=r?.data||r;if(!s?.signed_url)throw new Error('Could not create a private viewing link.'); window.open(s.signed_url,'_blank','noopener,noreferrer'); } catch(e){toast({title:'Could not open assignment',description:e.message,variant:'destructive'})} };
