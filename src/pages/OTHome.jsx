...
export default function OTHome(){
 const{user}=useAuth(),{toast}=useToast();const staffName=userDisplayName(user,'OT provider');
 ...
 const save=async()=>{
  ...
  try{
   await base44.entities.SessionRecord.create({
    student_id:f.student_id,date:f.date,provider:staffName,
 ...
 return <div className="space-y-6">
  ...
  <div className="flex justify-end"><AnalyticsPdfButton title="CaseCue OT Analytics" subtitle={staffName} filename="casecue-ot-analytics" metrics={[{label:'Sessions',value:sessions.length},{label:'Minutes delivered',value:totalMinutes},{label:'Average independence',value:`${avg}%`}]} charts={[{type:'line',title:'Independence & Service Trend',subtitle:'Saved OT independence and delivered minutes.',data:trend,series:[{key:'independence',label:'Independence %'},{key:'minutes',label:'Minutes'}]},{type:'donut',title:'Support Level Mix',subtitle:'Documented support levels across OT sessions.',data:supportData}]} notes={['School-based OT data summary. Provider review is required before official reporting.']}/></div>
