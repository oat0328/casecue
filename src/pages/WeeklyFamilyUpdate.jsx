...
export default function WeeklyFamilyUpdate(){
 ...
 const familyPdfMetrics=metricRows.filter(x=>x.value!=='N/A');
 const pdfSections=current&&covered?[{heading:'Family Message',body:message},{heading:'Strengths',body:(current.strengths||[]).map(x=>'• '+x).join('\n')},{heading:'Current Focus',body:(current.focus_areas||[]).map(x=>'• '+x).join('\n')},{heading:'Next Steps',body:(current.next_steps||[]).map(x=>'• '+x).join('\n')}]:[];
 ...
 return <div className="space-y-6">
  ...
  {current&&<>
   ...
   <Card className="overflow-hidden">
    <div className="border-b bg-slate-50 p-6"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><div className="text-xs font-black uppercase tracking-[.16em] text-blue-700">{covered?'AI-generated draft · grounded in saved CaseCue data':'Data coverage check · AI narrative skipped'}</div><h2 className="mt-1 text-2xl font-black">{current.student_name} · {fmtDate(current.week_start)} to {fmtDate(current.week_end)}</h2></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={()=>save()}><Save className="mr-2 h-4 w-4"/>Save Edits</Button><Button variant="outline" onClick={()=>save('ready')} disabled={!covered}><CheckCircle2 className="mr-2 h-4 w-4"/>{workspace==='para'?'Ready for Educator Review':'Mark Ready'}</Button>{covered&&<AnalyticsPdfButton label="Family PDF + Graphs" title="CaseCue Weekly Family Update" subtitle={current.student_name+' · '+fmtDate(current.week_start)+' - '+fmtDate(current.week_end)} filename={filename} metrics={familyPdfMetrics} charts={pdfCharts} sections={pdfSections} notes={[]}/>} </div></div></div>
    <div className="space-y-4 p-6">
     ...
     <div className="flex flex-wrap gap-2">{covered&&<><Button onClick={copy}><Copy className="mr-2 h-4 w-4"/>Copy Family Message</Button><Button variant="outline" onClick={email}><Mail className="mr-2 h-4 w-4"/>Open Family Email</Button></>}<Button variant="outline" onClick={generate} disabled={busy}><RefreshCcw className="mr-2 h-4 w-4"/>Regenerate from Current Data</Button></div>
    </div>
   </Card>
