  ...
  <Tabs defaultValue="create">
   ...
   <TabsContent value="create" className="space-y-4">
    <Card className="p-6">
     ...
     <div className="mt-4 grid gap-3 md:grid-cols-[150px_1fr_auto_auto]"><div><div className="text-xs font-black">K-12 grade</div><select className="mt-1 h-10 w-full rounded-lg border bg-white px-3 text-sm" value={gradeLevel} onChange={e=>setGradeLevel(e.target.value)}><option value="">Choose grade</option>{GRADES.map(g=><option key={g} value={g}>{g==='K'?'Kindergarten':`Grade ${g}`}</option>)}</select></div><div><div className="text-xs font-black">Assessment example style</div><select className="mt-1 h-10 w-full rounded-lg border bg-white px-3 text-sm" value={templateStyle} onChange={e=>setTemplateStyle(e.target.value)}>{STYLES.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div><div><div className="text-xs font-black">Items per area</div><Input className="mt-1 w-28" type="number" min="3" max="10" value={itemsPerDomain} onChange={e=>setItemsPerDomain(e.target.value)}/></div><div className="self-end"><Button onClick={generate} disabled={busy||!studentId}>{busy?<Loader2 className="mr-2 h-4 w-4 animate-spin"/>:<ClipboardCheck className="mr-2 h-4 w-4"/>}Create Baseline</Button></div></div>
    </Card>
