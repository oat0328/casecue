...
ok('Weekly family update shared route exists',app.includes('path="/weekly-contact"')&&app.includes('path="/w/gen_ed/weekly-contact"')&&app.includes('path="/w/:workspace/weekly-contact"'),'shared family update route is missing');
ok('Family Friday is available across applicable workspaces',['para','speech','ot','nurse','psych','substitute','pe'].every(k=>caps.includes("'/w/"+k+"/weekly-contact'")),'shared Family Friday navigation missing');
