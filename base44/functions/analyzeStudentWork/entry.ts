...
export default async function(req) {
  ...
  try {
    ...
    if(isPara){
      const assigned=(await base44.entities.ParaStudentAccess.filter({student_id:studentId,active:true},'-updated_at',1))?.[0];
      if(!assigned)return Response.json({error:'This student is not assigned to your Para account.'},{status:403});
    }
    ...
    const goalId = isPara ? '' : String(body.goal_id || '');
    let goal = null;
    if (goalId) {
