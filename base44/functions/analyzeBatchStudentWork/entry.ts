...
const recomputeRubric=(rows=[])=>{const usable=(Array.isArray(rows)?rows:[]).filter(r=>Number(r.possible)>0&&Number.isFinite(Number(r.earned)));if(!usable.length)return null;const earned=usable.reduce((n,r)=>n+Number(r.earned||0),0),possible=usable.reduce((n,r)=>n+Number(r.possible||0),0);return possible>0?{earned,possible,percentage:Math.round(earned/possible*1000)/10,count:usable.length}:null;};
...
export default async function(req){
 try{
  ...
  for(const x of data.items||[]){
    ...
    else if(rubric){const rt=recomputeRubric(x.rubric_breakdown||[]);if(rt){x.score_earned=rt.earned;x.score_possible=rt.possible;x.percentage=rt.percentage;x.scoring_confidence='medium';x.scoring_basis=`${x.scoring_basis||''} Score recomputed from teacher-provided rubric criteria.`.trim();}else if(x.score_possible>0)x.percentage=Math.round((Number(x.score_earned||0)/Number(x.score_possible))*1000)/10;else x.percentage=0;}
