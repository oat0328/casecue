...
const recomputeFromRows=(rows=[])=>{const usable=(Array.isArray(rows)?rows:[]).filter(r=>Number(r.possible)>0&&Number.isFinite(Number(r.earned)));if(!usable.length)return null;const earned=usable.reduce((n,r)=>n+Number(r.earned||0),0),possible=usable.reduce((n,r)=>n+Number(r.possible||0),0);return possible>0?{earned,possible,percentage:Math.round(earned/possible*1000)/10}:null;};
...
export default async function(req){
 try{
  ...
  for(const x of data.items||[]){
    ...
    const basis=String(x.identity_basis||'');
    const returned=students.find(v=>v.id===x.student_id);
    const ranked=students.map(s=>({s,score:nameScore(visibleName,`${s.first_name||''} ${s.last_name||''}`)})).sort((a,b)=>b.score-a.score);
    const best=ranked[0],second=ranked[1];
    let resolved=returned||null;
    if(visibleName&&best?.score>=.86&&(!second||best.score-second.score>=.08))resolved=best.s;
    if(resolved){
      const rosterName=`${resolved.first_name||''} ${resolved.last_name||''}`.trim();
      const visibleScore=nameScore(visibleName,rosterName);
      const basisNorm=normName(basis),first=normName(resolved.first_name||''),last=normName(resolved.last_name||'');
      const basisNamesStudent=(first&&basisNorm.includes(first))||(last&&basisNorm.includes(last));
      if(visibleScore>=.86||basisNamesStudent){x.student_id=resolved.id;x.student_name=visibleName||rosterName;x.student_match_confidence='high';x.identity_basis=basis||`Visible name matched roster: ${visibleName||rosterName}`;}
      else if(returned&&String(x.student_match_confidence)==='high'&&visibleName){x.student_id=resolved.id;x.student_match_confidence='medium';x.cautions=[...(x.cautions||[]),`Visible name '${visibleName}' needs teacher confirmation for ${rosterName}.`];}
      else{x.student_id='';x.suggested_goal_id='';x.student_match_confidence='unmatched';x.goal_match_confidence='none';}
    }else{x.student_id='';x.suggested_goal_id='';x.student_match_confidence='unmatched';x.goal_match_confidence='none';}
