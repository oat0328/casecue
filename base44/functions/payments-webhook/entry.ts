...
async function handleOrderApproved(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Grant the Founding Teacher subscription: set `plan` on the buyer's User record.
  // A plain field update keyed on the user id — naturally idempotent, safe on duplicate delivery.
  // Prefer the signed-in buyer's app user id; fall back to matching the buyer's email from the
  // Wix checkout page (never an empty string — it would match every orphan row).
  let grantUserId: string | null = purchase.appUserId ?? null;
  if (!grantUserId && buyerEmail) {
    const matchingUsers = await db.entities.User.filter({ email: buyerEmail });
    grantUserId = matchingUsers?.[0]?.id ?? null;
  }
  if (grantUserId) {
    ...
    // (invite-only), so the purchase row keeps buyerEmail for reconciliation at signup.
    console.warn("payments-webhook: no app user to grant yet", { checkoutId, buyerEmail });
  }
  // ===== END APP-SPECIFIC =====
...
async function handleSubscriptionEnded(db: any, eventData: any): Promise<Response> {
  ...
  // ===== APP-SPECIFIC =====
  // Revoke the Founding Teacher plan (mirror of the grant). Idempotent field update.
  let revokeUserId: string | null = purchase.appUserId ?? null;
  if (!revokeUserId && purchase.buyerEmail) {
    const matchingUsers = await db.entities.User.filter({ email: purchase.buyerEmail });
    revokeUserId = matchingUsers?.[0]?.id ?? null;
  }
  if (revokeUserId) {
